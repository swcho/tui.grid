...
export declare type ColumnDefaultValues = {
    name: string;
    value: CellValue;
}[];
export declare type CellValue = number | string | boolean | null | undefined;
export declare type Range = [number, number];
export declare type Side = 'L' | 'R';
export declare type SelectionType = 'cell' | 'row' | 'column';
export declare type SelectionUnit = 'cell' | 'row';
export declare type VisibleColumnsBySide = {
    [key in Side]: ColumnInfo[];
};
export declare type RowKey = number | string;
export declare type CellIndex = [number, number];
export declare type GridId = number;
export declare type EditingEvent = 'click' | 'dblclick';
export declare type State = 'DONE' | 'EMPTY' | 'LOADING';
export interface Dictionary<T> {
    [index: string]: T;
}
export declare type Row = Dictionary<CellValue> & {
    rowKey: RowKey;
    rowSpanMap: RowSpanMap;
    _attributes: RowAttributes;
};
export declare type RowSpanMap = Dictionary<RowSpan>;
...
export interface RowAttributes {
    ...
    className: {
        row: string[];
        column: Dictionary<string[]>;
    };
    height?: number;
    tree?: TreeRowInfo;
    expanded?: boolean;
}
export declare type RowAttributeValue = RowAttributes[keyof RowAttributes];
export declare type SummaryPosition = 'top' | 'bottom';
export declare type SummaryColumnContent = SummaryColumnContentMap | null;
export declare type SummaryColumnContents = Dictionary<SummaryColumnContent>;
export declare type SummaryValues = Dictionary<SummaryValue>;
export declare type CustomValue = string | ((value: CellValue, rowAttrs: Row[], column: ColumnInfo) => string);
export interface ClipboardCopyOptions {
    useFormattedValue?: boolean;
    useListItemText?: boolean;
    customValue?: CustomValue;
}
export declare type ValidationType = 'REQUIRED' | 'TYPE_STRING' | 'TYPE_NUMBER';
...
export declare type Formatter = ((props: FormatterProps) => string) | string;
...
export declare type RangeAreaInfo = {
    [key in Side]: AreaInfo | null;
};
export declare type RangeBySide = {
    [key in Side]: {
        row: Range;
        column: Range | null;
    };
